...
def main():
    parser = argparse.ArgumentParser(description="Compile Souffle algorithms")
    ...
    parser.add_argument("--compiler",
                        default=repo_root.parent / "souffle" / "build" / "src" / "souffle",
                        type=Path,
                        help="Path to the souffle binary")
    ...
    if not args.compiler.is_file():
        print(f"Error: souffle compiler not found: {args.compiler}", file=sys.stderr)
        sys.exit(1)
    ...
    for name in algo_names:
        ...
        output_binary = args.output_dir / f"{name}-souffle"
        cmd = [
            str(args.compiler),
            "-D", ".",
            "-F", ".",
